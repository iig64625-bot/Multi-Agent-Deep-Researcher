...
def researcher_node(state: ResearchState) -> ResearchState:
    ...
    for subquestion in state.get("plan", []):
        ...
        sources_to_read = raw_sources[: settings.max_pages_per_query or len(raw_sources)]
        for source in sources_to_read:
            page_text = reader.read(source.get("url", ""))
        ...
        if not raw_sources:
            summary = "未配置 Tavily API Key 或搜索无结果，无法获取实时外部资料。"
        else:
            try:
                response = get_llm(temperature=0.2, settings=settings).invoke(
                    RESEARCH_SUMMARY_PROMPT.format(
                        subquestion=subquestion,
                        evidence=_format_evidence(enriched or raw_sources),
                    )
                )
                ...
                summary = "\n".join(
                    ...
                    for source in enriched or raw_sources
                )
